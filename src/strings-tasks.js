...
function removeLastOccurrences(str, value) {
  const searchIndex = str.lastIndexOf(value);
  if (searchIndex === -1) {
    return str;
  }
  return str.slice(0, searchIndex) + str.slice(searchIndex + value.length);
...
function countVowels(str) {
  const searchStr = /[aeiouyAEIOUY]/g;
  const count = str.match(searchStr);
...
function isPalindrome(str) {
  const removeStr = /[!,? ]/gi;
  const formatStr = str.toLowerCase().replaceAll(removeStr, '');
...
function encodeToRot13(str) {
  ...
  const encodeAl = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const searchPos = alphabet.indexOf(str[i]);
    if (searchPos === -1) {
      result += str[i];
    } else {
      result += encodeAl[searchPos];
    }
  }
  return result;
}
